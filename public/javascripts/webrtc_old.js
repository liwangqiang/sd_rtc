...
(function() {
   ...
   */
  rtc.connect = function(server,callback) {
  ...
  //创建房间
  rtc.createRoom=function(room,successCallback){
	rtc._socket.send(JSON.stringify({
				"eventName":"createRoom",
				"data":room
			}));
	rtc.on('createSuccess',function(data){
		console.log('create success');
		successCallback(data);
	});
  };
   //转入房间
 rtc.go=function(room){
	var url=room.id;
	if(url){
  ...
  //加入房间
  rtc.register = function (userName,callback) {
	if (typeof userName === 'string') {
		...
		rtc.on("registerSuccess",function(data){
			if(user.name===data.userName)
			{
				...
				callback((function(){
				   var currentUser={
				     name:user.name,
					 id:user.id,
					 join:function(roomId,successCallback){
							rtc._socket.send(JSON.stringify({
									"eventName": "joinRoom",
									"data": {
									'room':roomId,
							...
							rtc.on('joinSuccess',function(data){
								var room=data;
								successCallback(function(){
									rtc.on('userJoin',function(data){
										console.log("userJoin :"+data.id);
										room.onlineUsers.push(data.userId);
									});
									rtc.on('userOut',function(data){
										console.log("userOut :"+data.id);
									});
									rtc.on('receiveChat',function(data){
										console.log('receiveMessage '+data.message);
									});
									return {
										getUserList:function(){
											return room.onlineUsers;
										},
			...
			{
				console.log('注册时，返回的name不匹配,注册失败');
			}
		});
	}
  };
  rtc.getAllUsers=function(successCallback){
	rtc._socket.send(JSON.stringify({
				"eventName":"getAllUsers",
				"data":null
			}));
	rtc.on('getAllUsersSuccess',function(data){
		console.log('get success');
		successCallback(data);
	});
  };
  rtc.getAllRooms=function(successCallback){
	rtc._socket.send(JSON.stringify({
				"eventName":"getAllRooms",
				"data":null
			}));
	rtc.on('getAllRoomsSuccess',function(data){
	console.log('get success');
		successCallback(data);
	});
  };
